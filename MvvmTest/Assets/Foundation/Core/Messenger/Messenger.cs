...
{
    /// <summary>
    /// A message broker for global messages.
    /// </summary>
    public class Messenger
    {
        ...
        static void Invoke(Delegate handler, object message)
        {
            if (handler.Method.ReturnType == typeof(IEnumerator))
            {
                ...
                {
                    TaskManager.StartRoutine((IEnumerator)handler.Method.Invoke(handler.Target, new[] { message }));
                }
        ...
        public static void Subscribe(object instance)
        {
            lock (Subscriptions)
            {
                var methods = instance.GetType().GetMethods().Where(o => o.HasAttribute<SubscribeAttribute>()).ToArray();
        ...
        public static void Publish(Type mType, object m, object ignore)
        {
            ...
            if (mType.HasAttribute<CachedMessage>())
            {
                if (mType.GetAttribute<CachedMessage>().OnePerType)
                    ClearCache(mType);
                else
                    RemoveCache(m);
                AddCache(mType, m);
...
#if UNITY_EDITOR
            if (m.GetType().HasAttribute<CachedMessage>())
