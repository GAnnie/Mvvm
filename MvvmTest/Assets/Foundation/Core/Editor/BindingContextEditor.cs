...
namespace Assets.Foundation.Core.Editor
{
    ...
    public class BindingContextEditor : UnityEditor.Editor
    {
        ...
        void ModeSelection()
        {
            var modes = Enum.GetValues(typeof(BindingContext.BindingContextMode)).Cast<BindingContext.BindingContextMode>().Select(o => o.ToString()).ToArray();
            var index = Array.IndexOf(modes, Target.ContextMode.ToString());
            var i = EditorGUILayout.Popup("Binding Mode", index, modes);

            if (i != index)
            {
                Target.ContextMode = (BindingContext.BindingContextMode)Enum.Parse(typeof(BindingContext.BindingContextMode), modes[i]);
        ...
        void MonoSelection()
        {
            Target.ViewModel = (MonoBehaviour)EditorGUILayout.ObjectField("ViewModel Componenet", Target.ViewModel, typeof(MonoBehaviour), true);
            if (Target.ViewModel != null)
                EditorGUILayout.LabelField(Target.ViewModel.GetType().ToString());
        }

        void DrawNamespaceDrop()
        {
        ...
        void DrawTypeDrop()
        {
            var list = BindingContext.ModelTypes.Where(o => o.Namespace == Target.ModelNamespace && !o.HasAttribute<HideInInspector>()).OrderBy(o => o.Name).ToArray();
