...
{
    /// <summary>
    /// Mediates between the model and the data-binders.
    /// 
    /// Use the MonoBinding to bind to a monobehaviour.
    /// Use the PropertyBinding to bind to a inner property of a parent model
    /// Use the MockBinding to bind against a model template (for item controls)
    /// </summary>
        ...
        public void FindModel()
        {
            switch (ContextMode)
            {
                ...
                case BindingContextMode.PropBinding:
                    Context = this.GetComponenetInParent<BindingContext>(true);
                    if (!Application.isPlaying)
                        SetPropertyTypeData();
                    break;
