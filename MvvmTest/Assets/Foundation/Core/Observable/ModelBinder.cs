...
{
    /// <summary>
    /// Implements IBindable on behalf of other objects.
    /// This allows the use of (most of) IBindable on non IBindable objects. 
                ...
                if (method.ReturnType == typeof(IEnumerator))
                {
                    if (_insanceBehaviour)
                    {
                        if (!_insanceBehaviour.gameObject.activeSelf ||
                            !_insanceBehaviour.enabled)
                        {
                            Debug.LogError("Behaviour is inactive !");
                        }
